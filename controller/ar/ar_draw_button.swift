...
class ARAlertView : SCNNode {
    ...
    func add(to sceneView : ARSCNView,x:Float,y:Float) {
        ...
        let ptrotation=sceneView.pointOfView?.rotation
        alertNode.rotation=SCNVector4((ptrotation?.x)!, (ptrotation?.y)!,(ptrotation?.z)!, (ptrotation?.w)!)
