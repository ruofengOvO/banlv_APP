...
//        btn.titleLabel?.font = UIFont(name: "HelveticaNeue", size: 11)
    ...
    func initializeViews() {
        ...
        passwordLabel = createLabel(text: "密码")
        ...
        repeatPasswordLabel = createLabel(text: "再次输入密码")
        ...
        createAcntBtn = createButton(title: "注册")
        ...
        // sign in view's elements
        navigateSignInBtn = createButton(title: "登录")
        ...
        emailLabel2 = createLabel(text: "邮箱")
        ...
        passwordLabel2 = createLabel(text: "密码")
        ...
        forgotPasswordBtn = createButton(title: "忘记密码?")
        ...
        signInButton = createButton(title: "登录")
    ...
    func juge(id:String,pass:String)->String
    {
        do{
            let consql=ConnectSQL()
            consql.tableName="UserInfo"
            consql.Conected()
            let query = OHMySQLQueryRequestFactory.select(consql.tableName, condition: "userEmail = '\(id)'")
            let reaponse = try consql.context.executeQueryRequestAndFetchResult(query)
            print(reaponse)
            if reaponse.isEmpty==true{return "ni"}
            else{
            let passu = (reaponse.first!["userPassword"] as! NSString) as String
            let idu = (reaponse.first!["userEmail"] as! NSString) as String
                consql.Closed()
            print(passu)
            if passu==pass{
                return "sc"
    ...
    func creat(id:String,pass:String)->String{
        let ju=juge(id: id, pass: pass)
        if ju=="ni"{
        do{
            let consql=ConnectSQL()
            consql.tableName="UserInfo"
            consql.Conected()
            let query2 = OHMySQLQueryRequestFactory.insert(consql.tableName, set: ["userID":"String","userName":"String","userPassword":pass,"userTel":" ","userEmail":id,"userCredit":"String","userState":"String"])
            try consql.context.execute(query2)
            consql.Closed()
            return "sc"
    ...
    @objc func handleCreateAccount(_ sender: UIButton) {
        ...
        if password != repeatPassword {
            alertWithMessage(message: "密码不匹配")
            
        }
        else{
//            let defaults = UserDefaults.standard
//            defaults.set(password, forKey: email)
            let cr=creat(id: email, pass: password)
            if cr=="ie"{
                alertWithMessage(message: "用户名已存在")
            }
            if cr=="sc"{
                 OKWithMessage(message: "注册成功")
            }
            if cr=="we"{
                alertWithMessage(message: "未知错误")
            }
        }
    ...
    @objc func handleSignIn(_ sender: UIButton)  {
        ...
        let defaults = UserDefaults.standard
        let ju=juge(id: email, pass: password)
        if ju=="ni"{
            alertWithMessage(message: "用户名不存在")
        }
        if ju=="pe"{
            alertWithMessage(message: "密码错误")
        }
        if ju=="sc"{
               defaults.set(email, forKey: "tag")
...
//        if (isLogin(in_email: email, in_password: password)) {
         //   self.signInSuccesfully()
//        }
//        else{
//            alertWithMessage(message: "密码错误")
//        }
    ...
    func isLogin(in_email:String , in_password:String) -> Bool{
        let defaults = UserDefaults.standard
        let temp_password = defaults.string(forKey: in_email)
        print(temp_password)
        if temp_password==in_password {
            defaults.set(in_email, forKey: "tag")
            return true
    ...
    func signInSuccesfully() {
        DispatchQueue.main.asyncAfter(deadline: .now() + 2.4) {
            ...
            let vc=sb.instantiateViewController(withIdentifier: "Launch_iden")
...
//            loginSuccess.modalPresentationStyle = .fullScreen
    ...
    func OKWithMessage(message: String) {
        DispatchQueue.main.asyncAfter(deadline: .now() + 2.4) {
            let alert = OKViewController(message: message)
            alert.modalTransitionStyle = .crossDissolve
            alert.modalPresentationStyle = .overCurrentContext
    ...
    func animateViewsWithFlag(flag: Bool) {
        ...
        UIView.animate(withDuration: 0.1, delay: 0, options: [.curveLinear, .allowUserInteraction], animations: {
            ...
            UIView.animate(withDuration: 0.3, delay: 0, usingSpringWithDamping: 1, initialSpringVelocity: 1, options: [.curveLinear, .allowUserInteraction], animations: {
                self.view.layoutIfNeeded()
                self.view.bringSubviewToFront(flag ? self.signInView : self.createAccountView)
