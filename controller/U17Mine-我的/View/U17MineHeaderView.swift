...
class U17MineHeaderView: UITableViewHeaderFooterView {
  ...
  private lazy var tagView : TagView = {
    ...
    lazy var loginLabel : UILabel = {
        let label = UILabel()
        let lab=UserDefaults.standard.value(forKey: "tag") as!String
        label.text = lab
    ...
    lazy var subloginLabel : UILabel = {
        let label = UILabel()
        label.text = NSLocalizedString("开通会员可使用更多精彩功能", comment: "开通会员可使用更多精彩功能")
        label.textColor = UIColor.white
...
private class TagView: UIView {
    ...
    private let titleArray = ["足迹","动态","余额"]
    ...
    lazy var vipLabel : UILabel = {
        let label = UILabel()
        label.text = NSLocalizedString("VIP状态:未开通", comment: "VIP状态:未开通")
    ...
    lazy var signBtn : UIButton = {
        ...
        button.setTitle(NSLocalizedString("退出", comment: "退出"), for: UIControl.State.normal)
        button.backgroundColor = DominantColor
        button.layer.masksToBounds = true
        button.layer.cornerRadius = 15
        button.titleLabel?.font = UIFont.systemFont(ofSize: 17)
        button.addTarget(self, action: #selector(goOutBtnClick(button:)), for: UIControl.Event.touchUpInside)
        return button
    }()
    ...
    func createBtn(){
        ...
        for index in 0..<titleArray.count {
            ...
            label.text = NSLocalizedString(titleArray[index], comment: titleArray[index])
