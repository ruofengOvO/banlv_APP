...
- (void)loadImageFrmUrl:(NSString *)url placeholderImage:(UIImage *)image contentMode:(UIViewContentMode)mode{
    [self cancelCurrentImageLoad];
    ...
    if (url) {
        __weak UIImageView *wself = self;
        id <SDWebImageOperation> operation = [SDWebImageManager.sharedManager downloadWithURL:[NSURL URLWithString:url] options:0 progress:nil completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished) {
            if (!wself) return;
            dispatch_main_sync_safe(^{
                if (!wself) return;
                if (image) {
