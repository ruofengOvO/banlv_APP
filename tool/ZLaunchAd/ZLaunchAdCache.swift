...
// MARK: - 清除指定url缓存
func ZLaunchAdClearDiskCacheWithImageUrlArray(_ urlArray: Array<String>) {
    if urlArray.count == 0 { return }
    DispatchQueue.global().async {
        for url in urlArray {
            let path = "\(cacheImagePath())/\(MD5_1(url))"
...
func saveImage(_ data: Data, url: URL, completion: ((Bool)->())?) {
    DispatchQueue.global().async {
        let path = "\(cacheImagePath())/\(MD5_1(url.absoluteString))"
